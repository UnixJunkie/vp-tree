...
let distances (dist: 'a distance) (p: 'a) (points: 'a list) =
  L.rev_map (dist p) points

let variance (mu: float) (xs: 'a list) =
...
let select_vp (dist: 'a distance) (points: 'a list) =
  match points with
  | [] -> assert(false)
  | [x] -> (x, 0.0, [])
  | x :: xs ->
    let vp, mu, _spread, others =
      L.fold_left (fun (curr_p, curr_mu, curr_spread, curr_others) p ->
          ...
          let dists = distances dist p others in
...
let rec create dist points =
  match points with
  | [] -> Empty
  | [x] -> new_node x 0. 0. 0. 0. 0. Empty Empty
  | _more ->
    let vp, mu, others = select_vp dist points in
    let dists = L.rev_map (fun p -> (dist vp p, p)) others in
    ...
    new_node vp lb_low lb_high middle rb_low rb_high
      (create dist lpoints) (create dist rpoints)

let rec find_nearest dist acc query tree =
  match tree with
  | Empty -> acc
  | Node { vp; lb_low; lb_high; middle; rb_low; rb_high; left; right } ->
    let x = dist vp query in
    let tau, acc' =
      match acc with
      | None -> (x, Some (x, vp))
      | Some (tau, best) ->
    ...
    if x < middle then
      ...
      | false, false -> acc'
      | true, false -> find_nearest dist acc' query left
      | false, true -> find_nearest dist acc' query right
      | true, true ->
        (match find_nearest dist acc' query left with
         | None -> find_nearest dist acc' query right
         | Some (tau, best) ->
           match find_nearest dist acc' query right with
           | None -> Some (tau, best)
           | Some (tau', best') ->
      ...
      | false, false -> acc'
      | true, false -> find_nearest dist acc' query right
      | false, true -> find_nearest dist acc' query left
      | true, true ->
        (match find_nearest dist acc' query right with
         | None -> find_nearest dist acc' query left
         | Some (tau, best) ->
           match find_nearest dist acc' query left with
           | None -> Some (tau, best)
           | Some (tau', best') ->
...
let nearest_neighbor dist query tree =
  match find_nearest dist None query tree with
